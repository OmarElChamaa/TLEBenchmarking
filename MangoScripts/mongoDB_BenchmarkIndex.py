...
collection.create_index([("unique_id", ASCENDING)])  # Créer un index sur le champ unique_id
...
# Prendre le nombre d'itérations en argument
if len(sys.argv) != 2:
...
end_time = time.time()
print(f"MongoDB Insertion Time with Indexing: {end_time - start_time} seconds")
...
end_time = time.time()
print(f"MongoDB Update Time with Indexing: {end_time - start_time} seconds")
...
end_time = time.time()
print(f"MongoDB Query Time with Indexing: {end_time - start_time} seconds")
